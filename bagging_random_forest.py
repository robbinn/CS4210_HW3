...
with open('optdigits.tes', 'r') as testingFile:
    ...
    for k in range(20):  # we will create 20 bootstrap samples here (k = 20). One classifier will be created for each
        # bootstrap sample
        bootstrapSample = resample(dbTraining, n_samples=len(dbTraining), replace=True)

        # populate the values of X_training and Y_training by using the bootstrapSample
        for i in range(len(bootstrapSample)):
            X_training.append(bootstrapSample[i][:-1])
            Y_training.append(bootstrapSample[i][-1])
        ...
        for i, testSample in enumerate(dbTest):
            ...
            # 0,0,0,0] this arrays will consolidate the votes of all classifier for all test samples
    ...
    # this is the number of decision trees that will be generated by
    ...
    correct = 0
    for db in dbTest:
        class_predicted = clf.predict([db[:-1]])
        if class_predicted == db[-1]:
            correct += 1
    print(correct)
